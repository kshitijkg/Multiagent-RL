...
class Scenario(BaseScenario):
    ...
    def reset_world(self, world):
        ...
        for agent in world.agents:
            ...
            agent.state.p_pos = (-self.world_radius + m, -self.world_radius) #self.np_rnd.uniform(-self.world_radius, -self.world_radius+0.3, world.dim_p)
            agent.state.p_vel = np.zeros(world.dim_p)
            agent.state.c = np.zeros(world.dim_c)
            m = m + 0.75
        #l_locations = np.array(self.random.sample(self.l_locations, len(world.landmarks))) - self.world_radius
        
        l_locations = [(-self.world_radius + 1, -self.world_radius + 5), (self.world_radius, -self.world_radius + 4), (self.world_radius, -self.world_radius)]
    ...
    #if collision -1
    #if all agents reach epsilon in diffrent landmarks +10 (Check the scale)
    #landmarks closer to where agents are
    #fixit: remove collision and see what happens 
    #fixit: static landmark static agent, only 1 agent, 1 landmark
    #fixit: remove local reward and check, or keep it a small constant 
    #fixit: 0.1 : local within epsilon, +10 if for a landmark there is a agent within epsilon
    #fixit: increasing episode length 
    def reward(self, agent, world):
        """
        Vectorized reward function
        Agents are rewarded based on minimum agent distance to each landmark, penalized for collisions
        reward = c1 * (distance of agent to closest landmark -- only given when within epsilon) + c2*(distance of landmark to closest agent)
        """
        rew, rew1, rew2 = 0, 0, 0
        ...
        if agent == world.agents[0]:
            ...
            a_pos2 = a_pos.repeat(len(world.agents), axis=0)
            dist = np.sqrt(np.sum(np.square(l_pos - a_pos1), axis=2))
            rew1 = np.min(dist, axis=1)
            rew1 = 2*(sum(rew1 < 0.75))
            #rew1 = np.sum(rew1[rew1 < 0.75])
            rew2 = np.min(dist, axis=0)
            rew = rew1 + 10*(sum(rew2 < 0.75))
            ...
            #     n_collide = (dist_a < self.collide_th).sum() - len(world.agents)
            #     rew -= n_collide
        self.num_agent = self.num_agent + 1
        if self.num_agent == 2:
            self.t = self.t + 1
        return rew
